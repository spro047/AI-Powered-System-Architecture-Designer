...
function UserNodeComponent({ data, selected }: { data: ArchitectureNodeData; selected?: boolean }) {
  return (
    <div
      className={`
        relative group
    ...
    >
      ...
      <span className="text-4xl leading-none mb-1">{data.icon || "👤"}</span>
      <span className="text-xs font-bold uppercase tracking-wide text-center leading-tight px-1">
        {data.label}
      </span>
