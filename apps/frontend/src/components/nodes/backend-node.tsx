...
function BackendNodeComponent({ data, selected }: { data: ArchitectureNodeData; selected?: boolean }) {
  return (
    <div
      className={`
        relative group
    ...
    >
      ...
      {/* Rack header */}
      <div className="flex items-center gap-2 px-3 py-1.5 bg-green-400 text-white border-b-4 border-neo-black rounded-t-[11px]">
        ...
        <span className="w-1.5 h-3 bg-green-200 rounded-sm" />
      </div>

      {/* Server rack bars */}
      <div className="px-3 py-2 space-y-1.5">
        <div className="h-2 bg-green-200 border-2 border-neo-black rounded" />
        <div className="h-2 bg-green-100 border-2 border-neo-black rounded" />
        <div className="flex items-center justify-between gap-1.5">
          <div className="h-2 bg-green-50 border-2 border-neo-black rounded flex-1" />
          {data.technology && (
            <span className="text-[10px] font-semibold text-neo-gray-600 uppercase whitespace-nowrap">
              {data.technology}
            </span>
          )}
        </div>
...
export const BackendNode = memo(BackendNodeComponent);
