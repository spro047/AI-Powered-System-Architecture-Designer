...
function CacheNodeComponent({ data, selected }: { data: ArchitectureNodeData; selected?: boolean }) {
  return (
    <div
      className={`
        relative group
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-yellow-400 border-b-4 border-neo-black rounded-t-[11px] text-black">
        <span className="text-lg leading-none">{data.icon || "⚡"}</span>
        <span className="font-bold text-xs uppercase tracking-wide truncate flex-1">
          {data.label}
        </span>
      </div>
      ...
      <div className="flex flex-col items-center gap-1 px-3 py-2">
        <div className="flex items-center gap-1">
          ...
          <span className="text-[10px] font-bold text-neo-gray-600 uppercase">Cache</span>
        </div>
        {data.technology && (
          <span className="text-[10px] font-semibold text-neo-gray-500 uppercase">
            {data.technology}
          </span>
        )}
      </div>
